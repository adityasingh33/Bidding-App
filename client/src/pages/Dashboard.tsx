...
export default function Dashboard() {
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-[pulse_1.5s_ease-in-out_infinite] opacity-60">
      <div className="flex justify-between items-end mb-8 pt-2">
        <div>
          ...
          <div className="h-4 w-72 bg-white dark:bg-white/5 rounded"></div>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1,2,3,4].map(i => <div key={i} className="bg-white dark:bg-white/5 h-32 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg"></div>)}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 h-96 bg-white dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg p-8">
           <div className="h-8 w-1/3 bg-slate-200 dark:bg-slate-800 rounded-md mb-8"></div>
           <div className="space-y-6 pl-2">
              {[1,2,3].map(i => (
                 <div key={i} className="flex gap-4">
                    <div className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-800 flex-shrink-0"></div>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-[fadeIn_0.5s_ease-out]">
      ...
      {/* Stats Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg relative overflow-hidden group hover:border-indigo-500/50 transition-colors">
          ...
          <p className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Total Bids</p>
          <div className="flex items-end gap-3">
            <span className="text-4xl font-black text-slate-900 dark:text-white">{totalBids}</span>
            <span className="text-indigo-400 text-sm font-medium mb-1">Placed</span>
          </div>
        </div>

        <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg relative overflow-hidden group hover:border-emerald-500/50 transition-colors">
          <div className="absolute -right-6 -top-6 w-24 h-24 bg-emerald-500/10 rounded-full blur-2xl group-hover:bg-emerald-500/20 transition-all"></div>
          <p className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Auctions Won</p>
          <div className="flex items-end gap-3">
            <span className="text-4xl font-black text-slate-900 dark:text-white">{totalWins}</span>
            <span className="text-emerald-400 text-sm font-medium mb-1 flex items-center gap-1"><Trophy className="w-4 h-4" /> Trophies</span>
          </div>
        </div>

        <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg relative overflow-hidden group hover:border-purple-500/50 transition-colors">
          ...
          <p className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Active Listings</p>
          <div className="flex items-end gap-3">
            <span className="text-4xl font-black text-slate-900 dark:text-white">{activeAuctions}</span>
            <span className="text-purple-400 text-sm font-medium mb-1">Selling</span>
          </div>
        </div>

        <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg relative overflow-hidden group hover:border-pink-500/50 transition-colors">
          ...
          <p className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Watchlist</p>
          <div className="flex items-end gap-3">
            <span className="text-4xl font-black text-slate-900 dark:text-white">{watchlistIds.length}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-4">
        {/* Activity Timeline */}
        <div className="lg:col-span-2 bg-white dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg p-6 sm:p-8">
            ...
            {recentBids.length > 0 ? recentBids.map((bid) => {
              ...
              return (
                <div key={bid.id} className="relative group">
                  <div className="absolute -left-9 mt-1.5 w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-900 border-2 border-indigo-500 flex items-center justify-center shadow-[0_0_10px_rgba(99,102,241,0.4)] z-10">
                    <div className="w-2 h-2 rounded-full bg-indigo-400"></div>
                  </div>
                  
                  <div className="bg-white dark:bg-slate-950/40 p-5 rounded-2xl border border-slate-300 dark:border-slate-800/50 hover:border-indigo-500/30 transition-colors shadow-sm ml-2 group-hover:-translate-y-1 duration-300">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 mb-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-500 bg-slate-100 dark:bg-slate-900 px-2 py-1 rounded-md">{dateString} | {timeString}</span>
                    ...
                    <Link to={`/auctions/${bid.auction.id}`} className="block text-lg font-bold text-slate-900 dark:text-white hover:text-indigo-300 transition-colors truncate">
                      {bid.auction.title}
                    </Link>
                  </div>
                </div>
