              ...
              <>
                ...
                <div className="relative" ref={notifRef}>
                  <button 
                    ...
                    className={`relative p-2.5 rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/50 ${isNotifOpen ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'text-slate-400 hover:text-white hover:bg-slate-800/80'}`}
                  >
                  ...
                  {/* Notifications Menu */}
                  {isNotifOpen && (
                    <div className="absolute right-0 mt-3 w-80 bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl shadow-black/50 py-2 z-50 transform origin-top-right transition-all animate-[fadeIn_0.2s_ease-out] flex flex-col max-h-[28rem] overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-3 border-b border-slate-700/50 mb-1 bg-slate-900/95">
                        ...
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((n) => (
                            <div 
                              ...
                              className={`px-4 py-3.5 border-b border-slate-800/50 cursor-pointer transition-colors ${!n.read ? 'bg-indigo-500/5 hover:bg-indigo-500/10' : 'hover:bg-slate-800/50 opacity-80'}`}
                            >
                ...
                <div className="relative" ref={profileRef}>
                  ...
                  {/* Dropdown Menu */}
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-3 w-56 bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl shadow-black/50 py-2 z-50 transform origin-top-right transition-all animate-[fadeIn_0.2s_ease-out]">
            ...
            {token && (
              <div className="relative">
                <button 
                   ...
                   className={`p-2 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50 ${isNotifOpen ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-400 hover:text-white hover:bg-slate-800/50'}`}
                >
                ...
                {isNotifOpen && (
                   <div className="absolute right-0 top-12 w-screen max-w-sm px-4 z-50 animate-[fadeIn_0.2s_ease-out]">
                      <div className="bg-slate-900/95 backdrop-blur-xl border border-slate-700/80 rounded-2xl shadow-2xl flex flex-col max-h-[60vh] overflow-hidden">
                         <div className="flex justify-between items-center px-4 py-3 border-b border-slate-700/50 bg-slate-800/30">
            ...
            {token ? (
              <>
                ...
                <div className="border-t border-slate-800/80 mt-6 pt-6 grid grid-cols-2 gap-3">
                  <Link to="/my-auctions" onClick={() => setIsMobileMenuOpen(false)} className="col-span-1 text-center px-4 py-3 rounded-xl text-sm font-bold text-slate-300 bg-slate-800/40 hover:bg-slate-800/80 border border-slate-700/50">Inventory</Link>
                  <Link to="/my-bids" onClick={() => setIsMobileMenuOpen(false)} className="col-span-1 text-center px-4 py-3 rounded-xl text-sm font-bold text-slate-300 bg-slate-800/40 hover:bg-slate-800/80 border border-slate-700/50">Bid History</Link>
                  <button
